"""
使用 Ray 分布式框架在多个节点上并行tokenize数据
...
RANDOM_SEED = 42  # 随机种子，用于文件列表shuffle
...
def generate_token_stats_json(file_token_stats, target_dir):
    """
    为每个最后一级文件夹生成token统计JSON文件
    
    Args:
        file_token_stats: 字典，key为目标文件路径，value为token数量
        target_dir: 目标根目录
    """
    logger.info("\n开始生成token统计JSON文件...")
    ...
    for file_path, token_count in file_token_stats.items():
        if file_path is None or token_count == 0:
            continue
        ...
        if folder_key not in folder_stats:
            folder_stats[folder_key] = {'files': {}, 'total_tokens': 0}
        
        # 记录该文件的token数
        folder_stats[folder_key]['files'][file_name] = token_count
        folder_stats[folder_key]['total_tokens'] += token_count
    
    # 为每个文件夹生成JSON文件
    success_count = 0
    failed_count = 0
    
    for folder_path, stats in tqdm(folder_stats.items(), desc="生成统计JSON"):
        try:
            # 构建JSON文件路径
            json_file = Path(folder_path) / "token_stats.json"
            ...
            logger.error(f"生成统计JSON失败: {folder_path}, 错误: {str(e)}")
            failed_count += 1
    
    logger.info(f"token统计JSON生成完成: 成功 {success_count}, 失败 {failed_count}")


def process_files_on_node(file_list, source_dir, target_dir, tokenizer_path, text_columns, num_workers):
    """
    在单个节点上使用多进程处理文件列表
    ...
    node_logger.info(f"文本字段查找顺序: {text_columns}")
    ...
    success_count = 0
    failed_files = []
    file_token_stats = {}  # 记录每个文件的token数量
    ...
    # 汇总结果
    for original_count, processed_count, failed_file, target_file_path, total_tokens in results:
        ...
        if failed_file is None:
            if original_count > 0 or processed_count > 0:
                ...
                # 记录文件的token统计
                if target_file_path:
                    file_token_stats[target_file_path] = total_tokens
        else:
    ...
    node_logger.info(f"总处理数据量: {total_processed:,}")
    
    # 计算总token数
    total_tokens = sum(file_token_stats.values())
    node_logger.info(f"总token数: {total_tokens:,}")
...
@ray.remote
def process_node_task(file_list, source_dir, target_dir, tokenizer_path, text_columns, num_workers):
    """
    Ray远程任务：在节点上处理数据
...
def submit_tasks_to_nodes(all_nodes, node_file_lists, source_dir, target_dir, 
                          tokenizer_path, text_columns, num_workers_per_node):
    """
    向所有节点提交处理任务
...
def aggregate_results(results):
    ...
    for idx, result in enumerate(results):
        ...
        # 计算该节点的总token数
        node_tokens = sum(result.get('file_token_stats', {}).values())
        ...
        logger.info(f"  处理数据量: {result['total_processed']:,}")
        logger.info(f"  总token数: {node_tokens:,}")
...
def main():
    """主函数"""
    logger.info("=" * 80)
    logger.info("Ray 分布式数据Tokenize工具")
    logger.info("=" * 80)
    ...
    ray.init(
        address="auto",
        runtime_env={
            "pip": ["zstandard", "transformers", "torch"]  # 自动在所有节点安装依赖
        }
    )
    ...
    try:
        ...
        # 汇总所有节点的token统计
        logger.info("\n汇总所有节点的token统计...")
        all_file_token_stats = {}
        for result in results:
            if result.get('status') == 'success':
                all_file_token_stats.update(result.get('file_token_stats', {}))
        
        # 生成token统计JSON文件
        generate_token_stats_json(all_file_token_stats, TARGET_DIR)
